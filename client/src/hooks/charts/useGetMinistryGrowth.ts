...
interface UseGetMinistryGrowthOptions {
  granularity: string;
...
interface UseGetMinistryGrowthReturn {
  ministries: any;
  loading: boolean;
  error: string;
  fetchMinistryGrowth: (
...
export const useGetMinistryGrowth = ({
  granularity,
  authorization,
  start = null,
  end = null,
  groupId = null,
  filterId = null,
}: UseGetMinistryGrowthOptions): UseGetMinistryGrowthReturn => {
  const [ministries, setMinistries] = useState(null);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string>("");

  const fetchMinistryGrowth = async (
    granularity: string,
    authorization?: "group" | "admin" | null,
  ...
  return { ministries, fetchMinistryGrowth, loading, error };
};

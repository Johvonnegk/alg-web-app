...
serve(async (req) => {
  ...
  try {
    const { discipleship } = await req.json();
    let userId;
    let { email, ...discipleshipNoEmail } = discipleship;
    if (email) {
    ...
    const { data: _discipleship, error: discipleshipError } = await supabase
      .from("discipleship")
      .insert({ ...discipleshipNoEmail, user_id: userId });

    if (discipleshipError)
      return errorResponse(
        discipleshipError
          ? discipleshipError.message
          : "Could not insert discipleship",
        400
      );

    return new Response(
      JSON.stringify({
        message: "Successfully inserted discipleship",
      }),

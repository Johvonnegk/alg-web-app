...
import { useUpdateMeeting } from "@/hooks/groups/useUpdateMeeting";
...
const MeetingManager = ({ meeting }: MeetingManagerProps) => {
  ...
  const getDefaultValues = () => ({
    date: new Date(meeting.date ?? new Date()),
    time: format(new Date(meeting.date ?? new Date()), "HH:mm"),
  ...
  const markAll = (val: AttendanceStatus) => {
    meeting.attendances.forEach((_, idx) => {
  ...
  const meetingFormSchema = React.useMemo(
    () => makeMeetingSchema(meeting.attendances.length),
  ...
  const submitMeeting = async (values: z.infer<typeof meetingFormSchema>) => {
    const result = await updateMeeting(values, meetingId, groupId);
    if (result.success) {
    ...
    <div>
      <Card className="border-stone-300">
        ...
        <CardContent className="flex flex-col gap-y-5">
          <Card
            className={
              "flex w-full py-5 justify-center gap-x-2 border-2 border-stone-300"
            }
          >
            <Form {...meetingForm}>
              <form
              ...
              >
                <CardContent>
                  ...
                  <div>
                    ...
                    {meeting.attendances.map((a, idx) => (
                      <div
                        key={a.users.user_id ?? idx}
                        id="attendance"
